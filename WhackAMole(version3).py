...
class WhackAMole(QWidget):
    ...
    def initUI(self):
        ...
        self.selectGridSize()
        self.score = 0
        #Number to limit and track the moles.
        self.active_moles = 0
        self.max_moles = 3
    ...
    def selectGridSize(self):
        #User selects the grid size of the game.
    ...
    def placeRandomMole(self):
        ...
        #Moles are now seperate and won't disappear if a different one is clicked.
    ...
    def startMoleTimer(self, row, col):
        ...
        #Moles now have seperate spawn rates and can appear in multiple instances.
    ...
    def resetGame(self):
        if hasattr(self, 'timer'):
            self.timer.stop()
        #Stops all the independant Mole timers.
    ...
    def SaveScoreToFile(self):
        try:
            #Finding current Directory of the script
            current_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(current_dir, "score.txt")
            
            #Opens the file and places the score in there.
